...
export default function MessagesPage() {
  ...
  const { data: messages = [], isLoading: messagesLoading, error: messagesError, refetch: refetchMessages } = useMessages();
  ...
  if (conversationsError || messagesError) {
    return (
      <ErrorFallback
        ...
        description="The inbox could not be loaded. Retry to refresh conversations and message history."
        onRetry={() => Promise.all([refetchConversations(), refetchMessages()])}
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      <motion.div variants={item} className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">A more polished communication workspace with room for real-time sync later.</p>
      </motion.div>

      <motion.div
        variants={item}
        className="grid grid-cols-1 overflow-hidden rounded-2xl border border-border/70 bg-card shadow-card lg:grid-cols-3"
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="relative">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/15 font-semibold text-foreground">
                      {conversation.avatar}
                    </div>
