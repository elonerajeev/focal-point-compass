import { LayoutDashboard, Search, Sparkles, BriefcaseBusiness, BadgeDollarSign, BarChart3, Shield } from "lucide-react";
...
const items = [
  ...
  },
  {
    ...
    label: "Finance",
    icon: BarChart3,
    getPath: (role: UserRole) => getFirstAccessiblePathForSection("finance", role),
    matches: (pathname: string) => pathname.startsWith("/finance"),
  },
  {
    key: "system",
    label: "System",
    icon: Shield,
    getPath: (role: UserRole) => getFirstAccessiblePathForSection("system", role),
    matches: (pathname: string) => pathname.startsWith("/system"),
  },
] as const;
...
export default function MobileBottomNav() {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-40 border-t border-border/70 bg-[linear-gradient(180deg,hsl(var(--card)_/_0.98),hsl(var(--card)_/_0.9))] px-3 pt-2 pb-[env(safe-area-inset-bottom,0.75rem)] shadow-[0_-14px_34px_hsl(222_36%_8%_/_0.12)] backdrop-blur-2xl md:hidden"
      role="navigation"
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex h-12 flex-col items-center justify-center gap-1 border transition",
                RADIUS.lg,
